...
lasso_spec <- logistic_reg(penalty = tune(), mixture = 0.5) |>
  set_engine("glmnet")
...
final_lasso <- finalize_workflow(lasso_workflow, best_penalty_1se)
...
coef_plot <- lasso_coefs |>
  ...
  coord_flip() +
  labs(
    title = "top 10 genes selected by lasso",
...
write_csv(final_train, "data/processed/lasso_elastic_train_data.csv")
write_csv(final_test, "data/processed/lasso_elastic_test_data.csv")

print(paste("lasso has selected", length(selected_genes), "genes."))


ggsave("plots/feature-selection/lasso_tuning_plot_elastic.png",
       plot = tuning_plot, width = 8, height = 6
)

ggsave("plots/feature-selection/lasso_coefs_elastic.png",
